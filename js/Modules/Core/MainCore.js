var ImagesInc_Core = (function (mainCore) {
  ...
  (function Core_initialize() {
    ...
    if (mainCore.debug) {
      console.log('%c Core Moudle has been initialized...', 'color:blue');
    }
  })();
  ...
  mainCore.loadPageDefinitionsFileAndCallBack = function (callbackFunc) {
    ...
    if (mainCore.Utilitizes &&
      ...
      pageDefinitionFileName &&
      pageDefinitionsFilePath) {
        mainCore.Utilitizes.Load_JS_CSS(pageDefinitionFileName, 'js', pageDefinitionsFilePath);

        if (mainCore.confirmFileLoad(pageDefinitionFileName, 'js')) {
          callbackFunc();
          mainCore.Utilitizes.Remove_JS_CSS(pageDefinitionFileName, 'js');
        }
        else {
          mainCore.log(
            3,
            'Cannot load file; from mianCore.loadPageDefinitionsFileAndCallBack'
          );
        }
      }
      else {
  ...
  mainCore.getCompoenentObjAndCallback = function (componentObjID, callbackFunc) {
    try {
      ...
      mainCore.log(
        3,
        'Component definition was not found; from mainCore.getComponentObjAndLoad ' + e.message
      );
    }
  };

  mainCore.loadPageDefinitionFromStorageAndCallBack = function (pageObjDefName, callbackFunc) {
    var pageDefinitionObj,
        args = arguments;
    ...
    if (!pageDefinitionObj && timerCounter < recursiveMaxCounter) {
      ...
      window.setTimeout(function () {
        mainCore.
        loadPageDefinitionFromStorageAndCallBack
          .apply(null, args);
      }, fileLoadDelayTime)
    }
  ...
  // if file has already been loaded, then nothing to do
  mainCore.loadCSSfileFromObjDef = function () {
  ...
  mainCore.loadComponentFilesAndInitializeWithCallBack = function (pageDefinitionObj, callbackFunc) {
    ...
    if (pageDefinitionObj && typeof pageDefinitionObj === 'object') {
      if (pageDefinitionObj.scriptPath) {
        mainCore.loadJSfileFromObjDefAndCallBack(pageDefinitionObj.scriptFile, pageDefinitionObj.scriptPath, function () {
          mainCore.initializeComponent(
            pageDefinitionObj.CompoenentID,
            loadedComponentCallbackFunc
          );
        });
      }
        ...
        mainCore.logA(
          2,
          'Cloud not load Component script file; from mainCore.loadComponentFilesAndInitializeWithCallBack'
        );
        return;
      }
      ...
      mainCore.log(
        3,
        'Component definitin was not found, cannot render page; from mainCore.loadComponentFilesAndInitializeWithCallBack'
      );
    } 
  };
  ...
  mainCore.loadPageDefinitions = function () {
    ...
    if (mainCore.Utilitizes &&
      ...
      mainCore.log(
        3,
        'Cannot load file; from mainCoreloadPageDefinitions'
      );
    }
  };
  ...
  mainCore.initializeAllComponents = function () {
    this.log(1, 'Inititalizing all components...', 'orange');
    ...
    try {
      ...
      this.log(3, 'APPLICATION CATASTOPHIC ERROR!' + e.name + ': ' + e.message);
    }
  ...
  mainCore.destroyAllComponents = function (removeFromDom) {
    this.log(1, 'Destroy all components...', 'orange');
...
var ImagesInc_Core = (function (Core) {
  ...
  var insertHTMLTxt = function (containerID, newStructure) {
    ...
    if (containerElem) {
      ...
      Core.log(
        3,
        'Cannot set the innerHTML of an unfound element; insertHTLTxt'
      );
    }
  };
  ...
  var applyElementCSSClass = function (elementID, className) {
    ...
    if (!className) {
      Core.log(
        3,
        'No class name has been provided, exiting moule!'
      );
...
// event related functionality augmenation
var ImagesInc_Core = (function (Core) {
  var $ = Core.jQuery;
      ...
      for (var i = 0; i < Core.registeredComponents.length; i++) {
        if (Core.registeredComponents[i].id === componentID) {
          if (Core.registeredComponents[i].events[eventType]) {
            ...
            Core.log(1, 'Event "' + eventType + '" for "' + componentID + '" component hhas been turned off', 'blue');

            return true;
          }
          else {
            Core.log(1, '"' + componentID + '" component was note registered for ' + 'Event "' + eventType + '"', 'orange');

            return false;
          }
        }
      }
      Core.log(3, '"' + componentID + '" component wass not found; unregisterCustomEvent');

      return false;
    }
    else {
      Core.log(3, ' incorrect parameters have been passed in, from unregisterCustomEvent');
    }
  };

  var unregisterAllCsutomEvents = function (componentID) {
  ...
  var addToHistory = function (dataObj) {
    ...
    if (!!(window.history && history.pushState)) {
      ...
      alert('Your browser needs to be upgraded to the latesst version');
  ...
  var getFromHistory = function (e) {
    ...
    if (!!(window.history && history.pushState)) {
      ...
      else if (e.originalEvent && e.originalEvent.state) {
  ...
  Core.unregisterAllCsutomEvents = unregisterAllCsutomEvents;
...
var ImagesInc_Core = (function (Core) {
  ...
  Core.loadPageByAjax = function (apiURL, QueryStr, callbackFunc, page, method) {
    if ($ && Core.jQueryAjaxEngine && Core.jQueryAjaxEngine.loadPageByAjax) {
      ...
      Core.log(3, 'Cannot make Ajax cal!; from loadPageByAjax');
    }
  };

  Core.getJSONObj = function (url, callbackFunc) {
    if ($ && Core.jQueryAjaxEngine.getJSONObj) {
