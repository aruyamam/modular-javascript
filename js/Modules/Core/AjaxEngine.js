...
ImagesInc_Core.jQueryAjaxEngine = (function () {
  ...
  self.returnResult = function (handler) {
    if (response)  {
      // return thhe result back to the caller
      handler(response);
    }
    else {
  ...
  self.loadPageByAjax = function (elemToAttackTo, pageSrc, sectionToExtractId, callbackFunc) {
  ...
  self.makeAjaxCallToServer = function (url, theQuery, method, handler) {
    ...
    $.ajax({
      ...
      error: function (request, type, errorThrow) {
        var errorMsg;
        ...
        self.showErrorMessage(errorMsg + errorThrow);
      },
