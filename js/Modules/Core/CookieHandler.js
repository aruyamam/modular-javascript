...
ImagesInc_Core.CookieHandler = (function () {
  ...
  self.setCookieLifeSpan = function (domainValue) {
  ...
  self.getEncode_Decode = function (optionallFlag) {
    if (!optionallFlag && optionallFlag !== false) {
      return cookieEncodeFlag;
    }
    else {
      return optionallFlag;
    }
  };
  ...
  self.resetCookieLifeSpan = function (cookieName, lifeSpan, Opt_decodeFlag) {
    self.createCookie(
      cookieName,
      self.getCookieValueAsString(
        cookieName, Opt_decodeFlag
      ),
      lifeSpan,
      null,
      null,
  ...
  self.populateCookie = function (cookieName, value, duration, Opt_encodeFlag) {
    ...
    // if there is a duration has been set for the cookie, then use that, if purposely setting the ookie with empty string, then it is session cookie
    if (duration) {
    ...
    else { // if the cookie exists, add the new value to the other values for thhe cookie, if not duplicate
  ...
  // Adds a value to the cookie, If the cookie does not exist, it returns null, if it does not have the value, returns false
  ...
  self.findValueInCookie = function (cookieName, value, Opt_decodeFlag) {
    ...
    if (valueArray) {
      ...
      return null; // the cookie did not exist;
    }
  };
  ...
  self.getCookieValueAsString = function (cookieName, Opt_decodeFlag) {
    ...
    var passedCookieName = cookieName,
        ...
        theStoredCookieValue = null,
        cookieList = self.getDomainCookieList(),
        encode_decodeFlag;
    ...
    for (var i = 0; i < cookieList.length; i++) {
      if (encode_decodeFlag) {
        theStoredCookie = decodeURIComponent(cookieList[i]);
      }
      else {
        theStoredCookie = cookieList[i];
      }
      ...
      if (theDecodedCookieName === passedCookieName) {
        theStoredCookieValue = theStoredCookie.substring((theStoredCookie.indexOf('=') + 1), theStoredCookie.length);

        if (encode_decodeFlag) {
          theStoredCookieValue = decodeURIComponent(theStoredCookieValue);
        }

        return theStoredCookieValue;
      }
    }
  ...
  self.getCookieValueAsArray = function (cookieName, Opt_decodeFlag) {
    if (!cookieName) {
      ImagesInc_Core.log(3, 'No cookie name provided to deletedCookie, exiting with error');
        ...
        if (cookieValueArray.length === 1) {
          ...
          for (var i = 0; i < cookieValueArray.length; i++) {
            if (cookieValueArray[i] === value) {
              ...
              self.createCookie(cookieName, cookieValueArray, cookieLifeSpan, null, null, null, encode_decodeFlag);
              cookieValueArray = null;
  ...
  self.deleteCookie = function (cookieName, Opt_decodeFlag) {
    if (!cookieName) {
      ImagesInc_Core.log(3, 'No cookie name provided to deleteCookie, exiting wit error');
  ...
  self.deleteAllCookies = function (Opt_decodeFlag) {
    ...
    // find all cookie files and then send them to thhe deleteCookie function
    for (var i = 0; i < cookieList.length; i++) {
      cookie = cookieList[i];

      while (cookie.charAt(0) == ' ') {
        cookie = cookie.substring(1, coolie.length);
      }
